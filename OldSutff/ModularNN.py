...
class PGNeuralNet:
                ...
                with tf.name_scope('HiddenFullyConnected') as scope:
                    ...
                    for x in self.layers[1:]:
                        with tf.name_scope("Layer_%d/" %i) as scope:
                            self.bias.append(tf.Variable(tf.zeros([1, x]), name="Bias"))
                            i += 1

                    i = 1
                    for x, y in zip(self.layers[:-1], self.layers[1:]):
                        with tf.name_scope("Layer_%d/" %i) as scope:
                            self.weight.append(tf.Variable(tf.random_normal([x, y]), name="Weight"))
                            i += 1
                ...
                with tf.name_scope("Cost/") as scope:
                    ...
                    if self.gradient_policy:
                        with tf.name_scope("GradientPolicy/") as scope:
                            ...
                            self.sum_actor_critic = tf.reduce_mean(sum_trajectory_probs * self.advantages)

                            # Maximizamos a recompensa
                            self.cost  = - self.sum_actor_critic
                            # self.cost  = - self.sum_actor_critic
    ...
    def debug(self, actions, observations, advantages):
        print("Advantages b4: %s" %advantages)
        ...
        print("Onehot: %s" %tf.one_hot(self.actions, self.layers[-1]).eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print("Prob dist: %s" %self.a_L.eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print("Prob da trajetoria: %s" %self.tp.eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print("Reduced sum da traj: %s" %self.stp.eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print("Trajetoria reduzida * adv: %s" %self.sum_actor_critic.eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print("Cost: %f" %self.cost.eval(feed_dict={self.a_0:observations, self.actions:actions, self.advantages:advantages}, session=self.sess))
        print(" ")
        # print("Gradients:")
        ...
        # self.backpropagate_trajectory(actions, observations, advantages)
